...
def export_panel_data():
    ...
    df_perf_calc = (
        df_perf.with_columns((0.5 * (pl.col("d1") + pl.col("d2"))).alias("c"))
        .with_columns((pl.col("c") >= 0.5).cast(pl.Float64).alias("poor"))
        .with_columns((pl.col("c") * pl.col("poor")).alias("ck"))
    )
    ...
    df_part_calc = (
        df_part.with_columns((0.5 * (pl.col("d1") + pl.col("d2"))).alias("c"))
        .with_columns((pl.col("c") >= 0.5).cast(pl.Float64).alias("poor"))
        .with_columns((pl.col("c") * pl.col("poor")).alias("ck"))
    )
    ...
    meta = {
        "perfect": {
            "est_H_diff": est_h_perf,
            "se_H_diff": se_h_perf,
            "var_H_diff": se_h_perf**2,
        },
        "partial": {
            "est_H_diff": est_h_part,
            "se_H_diff": se_h_part,
            "var_H_diff": se_h_part**2,
        },
    }
...
def export_vcov_wald_data():
    ...
    df = pl.DataFrame(rows)
    df = (
        df.with_columns((0.5 * (pl.col("i1") + pl.col("i2"))).alias("c"))
        .with_columns((pl.col("c") >= 0.5).cast(pl.Float64).alias("poor"))
        .with_columns((pl.col("c") * pl.col("poor")).alias("ck"))
    )
