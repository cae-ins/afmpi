...
def test_domains_conformity():
    ...
    for val in [v for v in ref["values"] if v["over"] == "region"]:
        ...
        assert row["est"] == pytest.approx(val["est"], abs=tol["est"]), f"Mismatch in {m} ({sub}) est"
        assert row["se"] == pytest.approx(val["se"], abs=tol["se"]), f"Mismatch in {m} ({sub}) se"
...
def test_domains_stata_mpitb_conformity():
    ...
    if not stata_ref.exists():
        pytest.skip("Stata mpitb reference file domains_stata.json is not present (PLAN.md §14.10/§14.13)")
